...
status_t list :: insert_after(data_t e_data, data_t n_data){
    node * e_node = 0;
...
status_t list :: insert_before(data_t e_data,data_t n_data){
    node * e_node = 0;
    e_node = search_node(e_data);
    if(e_node == 0)
        return LIST_INVALID_DATA;
...
std :: ostream &operator<<(std :: ostream &os,const list &list_obj){
    os << "[start] <->";
    for(
        node *run = list_obj.head_node -> next;
